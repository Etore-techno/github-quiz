...
const CONVERSATIONS = {
  A: {
    ...
    "Coach (Basket)": [
      ...
      { from: "Coach", text: "Rappel entraînement mercredi 16h30.", when: "Lun 16:10" },
...
// ---------- Apps rendering ----------
  ...
  function renderNotifications() {
    ...
    return `
      ${appSectionTitle("Notifications", "visibles depuis l’écran verrouillé")}
    ...
    return `
      <div class="profile-card-real">
        ...
        <div class="profile-main">
          ...
          <div class="profile-sub">${escapeHtml(o.classe)} • ${escapeHtml(o.college)}</div>
        </div>
      </div>

      <div class="settings-card">
        ${appSectionTitle("Informations personnelles")}
        <div class="info-line"><span>Adresse</span><b>${escapeHtml(o.adresse || "—")}</b></div>
        <div class="info-line"><span>Téléphone</span><b>${escapeHtml(o.telephone || "—")}</b></div>
        <div class="info-line"><span>Email</span><b>${escapeHtml(o.email)}</b></div>
      </div>

      <div class="settings-card">
        ${appSectionTitle("Appareil")}
        <div class="info-line"><span>Modèle</span><b>${escapeHtml(device.model || "Smartphone")}</b></div>
        <div class="info-line"><span>Système</span><b>${escapeHtml(device.os || "OS mobile")}</b></div>
        <div class="info-line"><span>Stockage</span><b>${escapeHtml(device.storage || "—")}</b></div>
        <div class="info-line"><span>Batterie</span><b>${escapeHtml(device.battery || "—")}</b></div>
      </div>
    `;
  }
    ...
    return `
      <div class="health-header">
        ...
        <div>
          ...
          <div class="muted">Accessible sans déverrouillage complet.</div>
        </div>
      </div>
      ...
      <div class="settings-card">
        ...
        <div class="health-grid">
          ...
          <div class="health-tile"><span>Informations</span><b>${escapeHtml(card.conditions || "—")}</b></div>
        </div>
      </div>
  ...
  function renderSocial() {
    ...
    const feed = (s.feed || []).map(p => {
      ...
      `).join("");
      const warn = p.flag === "cyber"
        ? `<span class="pill danger">⚠️ publication problématique</span>`
        : (p.flag === "privacy" ? `<span class="pill warn">⚠️ lieu et horaire visibles</span>` : "");
      return `
        <article class="social-post">
          <div class="post-head">
            ...
            <div class="post-meta">
              ...
              <div class="muted">${escapeHtml(p.date || "")}${p.location ? " • " + escapeHtml(p.location) : ""}</div>
            </div>
          </div>
    ...
    return `
      <div class="social-profile">
        ...
        <div class="profile-name">${escapeHtml(profile.pseudo || "mathis_g")}</div>
    ...
    const threads = (m.threads || []).map(t => `
      <button class="thread-row" type="button" data-msg-thread="${escapeHtml(t.title)}">
        ...
        <div class="thread-main">
          <div class="row"><div class="item-title">${escapeHtml(t.title)}</div><div class="muted">${escapeHtml(t.when || "")}</div></div>
    ...
    if (!UI.mail.authed) {
      return `
        <div class="mail-login-card">
          ...
          <div class="profile-sub">${escapeHtml(address)}</div>
    ...
    const ops = opsArr.map(o => `
      <div class="payment-row">
        ...
        <div class="payment-main">
          ...
          <div class="item-sub">${escapeHtml(o.when || "")}</div>
        </div>
      </div>
    `).join("");
