...
(function () {
  ...
  function loadProfile(key) {
    ...
    sbProfile.textContent = activeProfile.label;
  ...
  function pinValidate() {
    ...
    if (pinBuffer === String((activeProfile.owner.pin || activeProfile.pin)).trim()) {
  ...
  pwdOk.addEventListener("click", () => {
    ...
    if (typed.toLowerCase() === (activeProfile.owner.accountPassword || activeProfile.accountPassword).toLowerCase()) {
      unlockLevel = 2; // full
  ...
  function renderHome() {
    ...
    homeSub.textContent = guest ? "Compte verrouillé (mode invité)" : "Mode complet";
  ...
  recentsOverlay.addEventListener("click", (e) => {
