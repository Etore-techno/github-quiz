...
function calculerTailleTexteAutoParColonne(colonne, mode, callback) {  
    ...
    if (mode === "portrait") {
        switch (colonne) {
            case 2: tailleTexteBase = 2.8; break;
            case 3: tailleTexteBase = 2.8; break;
            default: tailleTexteBase = 0.2;     
        }
    } else { 
        switch (colonne) {
            case 2: tailleTexteBase = 1.4; break;
            case 3: tailleTexteBase = 1.4; break;
            default: tailleTexteBase = 0.2;
        }
    }
...
function positionnerZonesEtElements2() {
 ...
 window.exerciceData.tableauzone.forEach(zoneData2 => {
    ...
    if (
        (window.app.etape === 3 && zoneData2.colonne === 2) ||
        (window.app.etape === 4 && (zoneData2.colonne === 2 || zoneData2.colonne === 3))
    ) {
...
let bouton = document.getElementById("validate-2-button");
    document.querySelectorAll(".dropzone2").forEach(zone => {
        ...
        if ((bouton.textContent === "Suivant" && etapeActuelle === 3 && colonneZone === 2) || 
