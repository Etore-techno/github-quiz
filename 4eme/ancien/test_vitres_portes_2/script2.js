  const donneesVerres = {
    ...
    "bois_renforce": {
      titre: "La porte en bois renforcé :",
    ...
    "haute_performance": {
      titre: "La porte haute performance :",
...
function mettreAJourThumbnailActif(type) {
  const thumbnails = document.querySelectorAll('.thumbnail');
thumbnails.forEach(t => {
  t.classList.remove('active');
  if (t.dataset.id === type) {
...
// Sélectionne le thumbnail correspondant à la partie en cours
thumbnails.forEach(t => {
  if (t.textContent.trim().toLowerCase().includes(type.replace(/_/g, ' '))) {
    t.classList.add('active');
  }
});
  mettreAJourThumbnailActif(type);
  ...
  function afficherInfosporte(type) {
    ...
    document.getElementById("explication").textContent = data.explication;
    document.getElementById("historique").textContent = data.historique;
    document.getElementById("image-explication").src = `images/portes/explication_${type}.jpg`;
    document.getElementById("image-historique").src = `images/portes/historique_${type}.jpg`;
   const thumbnails = document.querySelectorAll('.thumbnail');
thumbnails.forEach(t => {
  if (t.dataset.id === type) {
    t.classList.add('active');
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const defaut = body.dataset.default;

  if (type === "verre") {
    afficherInfosverre(defaut);
  } else if (type === "porte") {
    afficherInfosporte(defaut);
  } else if (type === "famille") {
    afficherInfosfamille(defaut);
  }
});
